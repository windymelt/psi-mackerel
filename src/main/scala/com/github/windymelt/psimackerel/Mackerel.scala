...
class MackerelClient(apiKey: String)(using client: org.http4s.client.Client[IO]):
  import org.http4s._
  ...
  private def postReq(reqUri: Uri, json: io.circe.Json): IO[Unit] =
    ...
    ).withEntity(json) // Don't .toString(): it modifies Content-Type into text/plain

    client.expectOr[SuccessfulResponse](req){ res =>
        for
            msg <- res.body.compile.toVector.map(_.toArray).map(arr => String(arr))
  ...
  def defineGraph(defs: Seq[GraphDefinition])(using GraphDefinitionEncoder: Encoder[GraphDefinition]): IO[Unit] =
    ...
    postReq(uri"https://api.mackerelio.com/api/v0/graph-defs/create", defs.asJson)
  ...
  def postServiceMetrics(serviceName: String, metrics: Seq[ServiceMetric])(using ServiceMetricsEncoder: Encoder[ServiceMetric]): IO[Unit] =
    import org.http4s.implicits.uri
    import io.circe.syntax._
    postReq(uri"https://api.mackerelio.com/api/v0/services/" / serviceName / "tsdb", metrics.asJson)
...
object MackerelClient:
  ...
  // Encoder
  given EncoderForDateTime: Encoder[Instant] = ins => Json.fromDouble(ins.getEpochSecond()).get
  given EncoderForServiceMetric: Encoder[ServiceMetric] = deriveEncoder[ServiceMetric]
  given EncoderForMetric: Encoder[Metric] = deriveEncoder[Metric]
  given EncoderForGraphDefinition: Encoder[GraphDefinition] = deriveEncoder[GraphDefinition]
  // Decoder
  given DecoderForSuccessfulResponse: Decoder[SuccessfulResponse] = deriveDecoder[SuccessfulResponse]
